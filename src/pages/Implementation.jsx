...
import {
    ...
    AlertCircle, Loader2, Plus, Tag, User, Clock
} from "lucide-react";
...
const Implementation = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            // Fetch idea details and implementations in parallel
            const [ideaRes, implRes] = await Promise.all([
                axios.get(`${API}/api/ideas/${ideaId}`),
                axios.get(`${API}/api/ideas/${ideaId}/implementations`),
            ]);
    ...
    return (
        <main className="relative min-h-screen bg-slate-950 text-white overflow-hidden pb-20">
            ...
            <div className="relative z-10 container mx-auto px-4 sm:px-6 pt-24 sm:pt-32 max-w-7xl">
                ...
                <button
                    onClick={() => navigate(-1)}
                    className="inline-flex items-center text-sm font-bold text-slate-400 hover:text-emerald-400 mb-8 transition-colors group"
                >
                    ...
                    <>
                        ...
                        {idea && (
                            <motion.div
                                ...
                                className="mb-10 rounded-2xl border border-white/8 bg-slate-900/50 backdrop-blur-md p-6 sm:p-8"
                            >
                                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-5">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex flex-wrap items-center gap-2.5 mb-3">
                                            {idea.difficulty && (
                                                <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${diffBadge[idea.difficulty] ?? diffBadge.MEDIUM}`}>
                                        ...
                                        {idea.techStack && (
                                            <div className="flex flex-wrap gap-2 mb-5">
                                                {idea.techStack.split(",").map((t) => t.trim()).filter(Boolean).map((tech) => (
                                                    <span
                                                        key={tech}
                                                        className="flex items-center gap-1 rounded-full border border-emerald-500/20 bg-emerald-500/8 px-2.5 py-1 text-[11px] font-semibold text-emerald-400"
                                                    >
                                                        <Tag size={9} />
                                                        {tech}
                                                    </span>
                                    ...
                                    <motion.button
                                        ...
                                        className="flex shrink-0 items-center gap-2 self-start rounded-2xl bg-emerald-500 px-5 py-3 text-sm font-bold text-black shadow-[0_0_20px_rgba(16,185,129,0.25)] transition hover:bg-emerald-400 hover:shadow-[0_0_30px_rgba(16,185,129,0.4)]"
                                    >
                                        <Plus size={16} />
                        ...
                        <div className="mb-6">
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                                Browse how developers have built this idea. See code, stack, and approach.
                            </p>
                        </div>
                        ...
                        <div className="min-h-[30vh]">
                            {implementations.length === 0 ? (
                                <motion.div
                                    ...
                                    className="bg-slate-900/50 border border-white/5 rounded-3xl p-12 text-center max-w-2xl mx-auto mt-4 backdrop-blur-xl shadow-2xl"
                                >
